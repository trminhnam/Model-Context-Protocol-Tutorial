...
class Book(BaseModel):
    ...
    tags: list[str] = Field(
        default_factory=list, description="Tags associated with the book"
    )


class BookISBNInput(BaseModel):
    isbn: str = Field(
        ..., description="The ISBN of the book to be removed or retrieved"
    )
...
class LibraryManagement:
    ...
    def get_analyzing_book_messages(
        self, book: dict, query: str
    ) -> list[dict[str, str]]:
        return [
            {
                "role": "user",
...
async def serve() -> None:
    ...
    async def list_prompts() -> list[Prompt]:
        ...
        return [
            ...
            Prompt(
                ...
                arguments=[
                    PromptArgument(
                        name="book", description="The book to analyze.", required=True
                    ),
                    PromptArgument(
                        name="query",
    ...
    async def get_prompt(
        ...
        if name == "suggest_random_book":
            ...
            prompt_result = library.get_suggesting_book_title_by_abstract_prompt(
                **arguments
            )
            return GetPromptResult(
                description=prompt.description,
